...
const ChatWindow = ({ conversationId, messages, typing, onBack, isMobile }) => {
  ...
  if (!conversationId) {
    return (
      <div className="flex flex-col items-center justify-center h-full text-center px-6 bg-gray-50">
        ...
        <h3 className="text-xl font-semibold text-secondary mb-2">Your Messages</h3>
        <p className="text-secondary-lighter max-w-sm">Select a conversation from the list to start chatting with your connections.</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col h-full bg-white">
      {/* Header */}
      <div className="px-4 py-3 border-b bg-white flex items-center gap-3 sticky top-0 z-10 shadow-sm">
        {isMobile && onBack && (
          <button
            onClick={onBack}
        ...
        <div className="flex-1 min-w-0">
          <h2 className="font-semibold text-secondary truncate">{otherUser.firstName} {otherUser.lastName}</h2>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-2 bg-gray-50">
        {messages.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-center px-4">
            ...
            <p className="text-secondary-lighter text-sm">No messages yet. Say hi! 👋</p>
          </div>
        ) : (
          messages.map((m, idx) => {
            ...
            return (
              <div key={m._id}>
                {showTime && (
                  <div className="text-center text-xs text-secondary-lighter my-4">
                ...
                <div className={`flex ${isOwn ? 'justify-end' : 'justify-start'} mb-1`}>
                  <div className={`max-w-[75%] sm:max-w-md px-4 py-2.5 rounded-2xl shadow-sm ${
                    isOwn 
                      ...
                      : 'bg-white text-secondary border border-gray-100 rounded-bl-md'
                  }`}>
                    {m.isDeleted ? (
        ...
        <div className="flex items-end gap-2 max-w-4xl mx-auto">
          <div className="flex-1 relative">
            <textarea
              ...
              rows="1"
              className="w-full resize-none rounded-full px-4 py-3 pr-12 border border-gray-300 focus:border-primary focus:ring-1 focus:ring-primary text-sm max-h-32 overflow-y-auto"
              placeholder="Message..."
          ...
          <button
            ...
            className="flex-shrink-0 w-11 h-11 rounded-full bg-primary text-white flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed hover:bg-primary/90 transition-all active:scale-95"
