    ...
    <div className="overflow-y-auto flex-1">
          ...
          >
            ...
            <div className="flex-1 min-w-0 py-1">
              <div className="flex items-center justify-between mb-1">
                <h3 className={`font-semibold truncate pr-2 ${c.unreadCount > 0 ? 'text-secondary' : 'text-secondary'}`}>
                  {otherUser.firstName} {otherUser.lastName}
                </h3>
                {c.lastMessageAt && (
                  <span className={`text-xs flex-shrink-0 ${c.unreadCount > 0 ? 'text-primary font-medium' : 'text-secondary-lighter'}`}>
                    {formatLastMessageTime(c.lastMessageAt)}
                  </span>
                )}
              </div>
              <p className={`text-sm truncate ${c.unreadCount > 0 ? 'text-secondary font-medium' : 'text-secondary-lighter'}`}>
                {lastMessagePreview}
              </p>
            </div>
          </button>
      ...
      {conversations.length === 0 && (
        <div className="flex flex-col items-center justify-center py-16 px-6 text-center">
          ...
          <h3 className="text-lg font-semibold text-secondary mb-2">No messages yet</h3>
          <p className="text-secondary-lighter text-sm max-w-xs">Start chatting with your connections from the Feed or Connections page!</p>
        </div>
      )}
    </div>
  );
